export const getRandomCoordinatesInBoundingBoxes = (boundingBoxes, excludingBoxes, numberOfCoordinates) => {
    ...
    while (coordinates.length < numberOfCoordinates) {
        ...
        const coordinate = {
            x: Math.random() * (boundingBox.max.x - boundingBox.min.x) +
                boundingBox.min.x,
            y: Math.random() * (boundingBox.max.y - boundingBox.min.y) +
                boundingBox.min.y,
            z: Math.random() * (boundingBox.max.z - boundingBox.min.z) +
                boundingBox.min.z,
        };
        let isExcluded = false;
        excludingBoxes.forEach((excludingBox) => {
            if (coordinate.x > excludingBox.min.x &&
