...
import { DEFAULT_TRANSITION_EFFECT, TRANSITION_UNIFORM_CONFIG, TRANSITION_VARYING_CONFIG, } from "./transitonEffect.consts";
export const transitionEffect = (previousPointName, effectProps) => {
    const transitionEffectProps = formatVertexParameters(effectProps, DEFAULT_TRANSITION_EFFECT);
    const pointName = generateUniquePointName(VERTEX_EFFECT_POINT_NAMES.TRANSITION_POINT, POINT_PARENTS.TRANSITION);
    const uniformConfig = TRANSITION_UNIFORM_CONFIG;
    const varyingConfig = TRANSITION_VARYING_CONFIG;
    const { transformation, effectUniforms, effectVaryings, effectPointName, effectFunctions, effectAttributes, } = transitionEffectTransform(pointName, previousPointName, transitionEffectProps);
