...
import { NOISE_EFFECT_TYPES, NOISE_FUNCTIONS, NOISE_UNIFORMS, } from "./noise.consts";
export const noiseTransform = (noiseParameters) => {
    ...
    switch (noiseType) {
        ...
        default:
            ...
            return {
                ...
                uniformConfig: NOISE_UNIFORMS,
            };
    }
};
export const noiseCloudTransform = (effectStrength) => {
    return `// Generate random offset for each vertex (compute once)
  float randomOffsetX = noise3D(${VERTEX_POINT_NAME}.xyz + vec3(0.0, 0.0, 0.0) ) * ${shaderSafeFloat(effectStrength)} * uNoiseStrength;
  float randomOffsetY = noise3D(${VERTEX_POINT_NAME}.xyz + vec3(1.0, 1.0, 1.0))  * ${shaderSafeFloat(effectStrength)} * uNoiseStrength;
  float randomOffsetZ = noise3D(${VERTEX_POINT_NAME}.xyz + vec3(2.0, 2.0, 2.0))  * ${shaderSafeFloat(effectStrength)} * uNoiseStrength;
...
export const cloudTransform = (effectStrength) => {
    return `// Generate random offset for each vertex (compute once)
       float randomOffsetX = (fract(sin(dot(${VERTEX_POINT_NAME}.xyz, vec3(12.9898, 78.233, 45.543))) * 43758.5453) - 0.5) *  ${shaderSafeFloat(effectStrength)} * uNoiseStrength;
