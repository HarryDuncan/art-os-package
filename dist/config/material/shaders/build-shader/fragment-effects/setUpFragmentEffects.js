...
export const setUpFragmentEffects = (fragmentEffects) => {
    const { varyingConfigs, uniformConfigs, transformations, attributeConfigs, requiredFunctions, structConfigs, } = getFragmentColors(fragmentEffects);
...
export const getFragmentColors = (fragmentEffects) => {
    ...
    fragmentEffects.forEach((effect) => {
        const { varyingConfig, uniformConfig, transformation, requiredFunctions, attributeConfig, structConfigs = [], } = getFragmentEffects(effect);
...
const setUpInitialParameters = () => {
    const { varyingConfig, uniformConfig, transformation, attributeConfig } = defaultFragmentEffect();
